...
def get_new_dataloader(args, session):
    ...
    if args.dataset == 'cub200':
        # txt_path = "data/index_list/" + 'cub200-20' + "/session_" + str(session + 1) + '.txt'
...
def get_visualize_dataloader(args, index):

    if args.dataset == 'cifar100':
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True, index=index, base_sess=True)
    ...
    if args.dataset == 'cub200':
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True, index=index, base_sess=True)
    ...
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True, index=index, base_sess=True)
