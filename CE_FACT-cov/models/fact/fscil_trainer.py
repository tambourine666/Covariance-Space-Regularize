...
class FSCILTrainer(Trainer):
    ...
    def get_vis_loader(self, index,train=False):
        visloader = get_visualize_dataloader(self.args, index,train=train)
        return visloader
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                tsl, tsa, acc2now, acc_new, acc_old = self.test_intergrate(self.model, testloader, 0, args, session,
                                                                           validation=True)
                ...
                if session == 1:
                    ...
                    with open(os.path.join(args.save_path, 'results_old_new.txt'), 'a', encoding='utf-8') as f1:
                        f1.write(f'\nSpecs:{result_list}\n')
                        f1.write(
                            f'Session {session}:\t{acc2now}\tCurrent Avg Acc:{(tsa * 100)}\tNovel classes Avg Acc:{acc_new_all} Harmony Acc:{hmeans}\n')
                ...
                if session == args.sessions - 1:
                    ...
                    with open(os.path.join(args.save_path, 'results_old_new.txt'), 'a', encoding='utf-8') as f1:
                        f1.write(
                            f'Mean Acc for this run is: {"{:.2f}%".format(mean_acc)}\t Each Session Acc is{all_acc} Harmony Acc:{hmeans}\n')
                        print(
                            f'Mean Acc for this run is:{"{:.2f}%".format(mean_acc)}\n Each Session Acc is{all_acc} Harmony Acc:{hmeans}\n')
                # save model
                self.trlog['max_acc'][session] = float('%.3f' % (tsa * 100))
                save_model_dir = os.path.join(args.save_path, 'session' + str(session) + '_max_acc.pth')
                # torch.save(dict(params=self.model.state_dict()), save_model_dir)
    ...
    def test_intergrate(self, model, testloader, epoch, args, session, validation=True):
        ...
        with torch.no_grad():
            ...
            if session > 0:
                novel_classes_so_far = np.arange(args.num_classes)[
                                       args.base_class:(args.base_class + session * args.way)]
    ...
    def set_save_path(self):
        ...
        if self.args.cov_balance > 0:
            self.args.save_path = self.args.save_path + '-cov_balance_%.4f' % (
